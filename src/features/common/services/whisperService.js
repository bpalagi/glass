...
class WhisperService extends EventEmitter {
    ...
    isLiveServerRunning() {
        return this.liveServerReady && this.liveServerProcess && !this.liveServerProcess.killed;
    }

    async ensureLiveVenv() {
        const pythonPath = path.join(this.liveVenvPath, 'bin', 'python3');
        try {
            await fsPromises.access(pythonPath, fs.constants.X_OK);
            return true;
        } catch {
            console.log('[WhisperService] WhisperLive venv not found, creating...');
            // Try to find python3.11 first (best ML compat), then fall back
            let systemPython = null;
            for (const py of ['python3.11', 'python3.12', 'python3.13', 'python3']) {
                systemPython = await this.checkCommand(py);
                if (systemPython) break;
            }
            if (!systemPython) throw new Error('No Python 3 found for WhisperLive venv');

            console.log(`[WhisperService] Creating venv with ${systemPython}...`);
    ...
    async startLiveServer(model = 'small.en') {
        ...
        if (this._liveServerStartPromise) {
            console.log('[WhisperService] WhisperLive server startup already in progress, waiting...');
            return this._liveServerStartPromise;
        }
    ...
    async _doStartLiveServer(model = 'small.en') {
        ...
        console.log(`[WhisperService] Starting WhisperLive server on port ${this.liveServerPort} with model=${model}`);
        ...
        this.liveServerProcess.on('close', (code) => {
            console.log(`[WhisperService] WhisperLive server exited with code ${code}`);
            this.liveServerReady = false;
            this.liveServerProcess = null;
        });
    ...
    async stopLiveServer() {
        if (this.liveServerProcess && !this.liveServerProcess.killed) {
            console.log('[WhisperService] Stopping WhisperLive server...');
            this.liveServerProcess.kill('SIGTERM');
            // Give it a moment to shut down gracefully
            await new Promise(r => setTimeout(r, 1000));
            if (this.liveServerProcess && !this.liveServerProcess.killed) {
                this.liveServerProcess.kill('SIGKILL');
            }
        }
        this.liveServerReady = false;
        this.liveServerProcess = null;
