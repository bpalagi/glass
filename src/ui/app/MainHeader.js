...
export class MainHeader extends LitElement {
    ...
    handleMouseDown(e) {
        ...
        const interactiveSelector = 'button, a, input, select, textarea, .listen-button, .header-actions, .settings-button';
        for (const node of path) {
            if (node instanceof Element && typeof node.matches === 'function' && node.matches(interactiveSelector)) {
                return;
            }
        }
